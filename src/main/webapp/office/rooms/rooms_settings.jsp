...
<body>
	...
	<div class="row">
		...
		<main class="col-md-9 ml-sm-auto col-lg-10 px-md-4" role="main">
			<div>
				<h1>Room</h1>
				<p>Manage the hotel's rooms from here.
					prices.</p>
					<c:set var="hotel" value="${listHotel[0] }"></c:set>
					<c:if test="${hotel.room_count <= listRoom.size() }">
					<h3>You have added all <c:out value="${hotel.room_count }"></c:out> of your hotel. <a href="/HotelBookings/office/hotelsettings/edit-hotel?hid=1">Increase room number</a> in order to add more rooms.</h3>
					</c:if>
					<c:if test="${hotel.room_count > listRoom.size() }">
					<h2>
						...
						<c:forEach var="room" items="${listRoom}">
							<tr>
								...
								<c:forEach var="room_type" items="${listRoomTypes}">
									<c:if test="${room_type.id == room.rtid}">
									<td><c:out value='${room_type.type}' /></td>
									</c:if>
								</c:forEach>
								<td><c:out value="${room.name}" /></td>
								<td><c:if test="${room.first_floor == true }"><i class="fas fa-check"></i></c:if></td>
								<td><c:if test="${room.extra_bed == true }"><i class="fas fa-check"></i></c:if></td>
								<td><c:if test="${room.baby_crib == true }"><i class="fas fa-check"></i></c:if></td>
								<td><c:if test="${room.handycap == true }"><i class="fas fa-check"></i></c:if></td>
